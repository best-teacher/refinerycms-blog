...
module Refinery
  module StudentBlog
    class Post < ActiveRecord::Base
      ...
      has_many :comments, :dependent => :destroy, :foreign_key => :student_blog_post_id
      acts_as_taggable

      has_many :categorizations, :dependent => :destroy, :foreign_key => :student_blog_post_id
      has_many :categories, :through => :categorizations, :source => :student_blog_category
      ...
      validates :source_url, :url => { :if => 'Refinery::StudentBlog.validate_source_url',
                                      :update => true,
                                      :allow_nil => true,
                                      :allow_blank => true,
      ...
      class << self
        ...
        def uncategorized
          live.includes(:categories).where(Refinery::StudentBlog::Categorization.table_name => { :student_blog_category_id => nil })
        end
        ...
        def comments_allowed?
          Refinery::Setting.find_or_set(:comments_allowed, true, :scoping => 'student_blog')
        end

        def teasers_enabled?
          Refinery::Setting.find_or_set(:teasers_enabled, true, :scoping => 'student_blog')
        end

        def teaser_enabled_toggle!
          currently = Refinery::Setting.find_or_set(:teasers_enabled, true, :scoping => 'student_blog')
          Refinery::Setting.set(:teasers_enabled, :value => !currently, :scoping => 'student_blog')
        end
      end

      module ShareThis
        def self.enabled?
          Refinery::StudentBlog.share_this_key != "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
        end
      end
