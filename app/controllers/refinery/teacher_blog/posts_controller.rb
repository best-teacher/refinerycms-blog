module Refinery
  module TeacherBlog
    class PostsController < TeacherBlogController

      before_filter :find_all_teacher_blog_posts, :except => [:archive]
      before_filter :find_teacher_blog_post, :only => [:show, :comment, :update_nav]
      before_filter :find_tags

      respond_to :html, :js, :rss

      def index
        # Rss feeders are greedy. Let's give them every teacher_blog post instead of paginating.
        ...
        if (@comment = @post.comments.create(params[:comment])).valid?
          if Comment::Moderation.enabled? or @comment.ham?
            begin
              ...
              logger.warn "There was an error delivering a teacher_blog comment notification.\n#{$!}\n"
            end
          end

          if Comment::Moderation.enabled?
            flash[:notice] = t('thank_you_moderated', :scope => 'refinery.teacher_blog.posts.comments')
            redirect_to refinery.teacher_blog_post_url(params[:id])
          else
            flash[:notice] = t('thank_you', :scope => 'refinery.teacher_blog.posts.comments')
            redirect_to refinery.teacher_blog_post_url(params[:id],
