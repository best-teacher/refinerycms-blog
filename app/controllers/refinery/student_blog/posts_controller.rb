module Refinery
  module StudentBlog
    class PostsController < StudentBlogController

      before_filter :find_all_student_blog_posts, :except => [:archive]
      before_filter :find_student_blog_post, :only => [:show, :comment, :update_nav]
      before_filter :find_tags

      respond_to :html, :js, :rss

      def index
        # Rss feeders are greedy. Let's give them every student_blog post instead of paginating.
        ...
        if (@comment = @post.comments.create(params[:comment])).valid?
          if Comment::Moderation.enabled? or @comment.ham?
            begin
              ...
              logger.warn "There was an error delivering a student_blog comment notification.\n#{$!}\n"
            end
          end

          if Comment::Moderation.enabled?
            flash[:notice] = t('thank_you_moderated', :scope => 'refinery.student_blog.posts.comments')
            redirect_to refinery.student_blog_post_url(params[:id])
          else
            flash[:notice] = t('thank_you', :scope => 'refinery.student_blog.posts.comments')
            redirect_to refinery.student_blog_post_url(params[:id],
