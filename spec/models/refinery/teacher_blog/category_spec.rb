...
module Refinery
  module TeacherBlog
    describe Category do
      let(:category) { FactoryGirl.create(:teacher_blog_category) }

      describe "validations" do
        it "requires title" do
          FactoryGirl.build(:teacher_blog_category, :title => "").should_not be_valid
        end

        it "won't allow duplicate titles" do
          FactoryGirl.build(:teacher_blog_category, :title => category.title).should_not be_valid
        end
      end

      describe "teacher_blog posts association" do
      ...
      describe "#post_count" do
        it "returns post count in category" do
          2.times do
            category.posts << FactoryGirl.create(:teacher_blog_post)
          end
