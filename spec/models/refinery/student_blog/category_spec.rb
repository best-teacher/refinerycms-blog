...
module Refinery
  module StudentBlog
    describe Category do
      let(:category) { FactoryGirl.create(:student_blog_category) }

      describe "validations" do
        it "requires title" do
          FactoryGirl.build(:student_blog_category, :title => "").should_not be_valid
        end

        it "won't allow duplicate titles" do
          FactoryGirl.build(:student_blog_category, :title => category.title).should_not be_valid
        end
      end

      describe "student_blog posts association" do
      ...
      describe "#post_count" do
        it "returns post count in category" do
          2.times do
            category.posts << FactoryGirl.create(:student_blog_post)
          end
